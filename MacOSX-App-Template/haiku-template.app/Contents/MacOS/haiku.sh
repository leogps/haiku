...
if [ -z "$JAVACMD" ]
then
      osascript -e 'display dialog "Java is required to run this. Please install Java 11 or higher" buttons {"OK"} default button "OK"'
      exit 1;
fi
...
exec "$JAVACMD" -Duser.dir="$progdir" \
     ...
     -jar "$progdir/../Java/haiku-jar-with-dependencies.jar" \
      ...
      "-Xdock:name=\"Haiku Player\"" \
      "-Xdock:icon=\"$progdir/../Resources/haiku.icns\"" \
      "$@"
