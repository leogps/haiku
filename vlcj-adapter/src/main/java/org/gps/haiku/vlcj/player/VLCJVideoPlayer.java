...
import org.gps.haiku.vlcj.player.impl.FXPlayerFrameImpl;
...
public class VLCJVideoPlayer implements VLCJPlayer {
    ...
    private static final Logger LOGGER = LogManager.getLogger(VLCJVideoPlayer.class);

    public VLCJVideoPlayer(MediaPlayerFactory mediaPlayerFactory) {
        this.mediaPlayerFactory = mediaPlayerFactory;
        fxPlayerFrame = new FXPlayerFrameImpl();
        isFXPlayer = JavaVersionUtils.isGreaterThan6() && OSInfo.isOSMac();
        init();
	}
    ...
    public void registerDragAndDropEvent(DropTarget dropTarget) {
        if(vFrame == null) {
            LOGGER.error("Video Frame not initialized, cannot register drag and drop event.");
            return;
        }
...
//        }
        ...
        transparentWindow.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                LOGGER.debug("Overlay window activated...");
